...
/* Runs the kernel monitor for lab 0.
   Prompts user with ICS143A> and waits for user input.
   Parses that input and checks if it is "whoami".
   If whoami is given prints Lakshya Shrivastava.
   If exit is given we quit the shell.
   All other commands print invalid command.
*/
void kernel_shell(void)
{
  char input[256]; // buffer to store user input.
  int index;       // index to travers buffer
  ...
  while (true)
  {
    ...
    index = 0;
    // can be put into seperate helper function named getline.
    while (true)
    {
      char c = input_getc(); // get a single character from the console.
      if (c == '\n' || c == '\r') // check for enter key.
      {
        printf("\n");
      ...
      // echo character if it's printable
      if (index < sizeof(input) - 1 && c >= 32 && c <= 126) // 32-126 is ascii printable range
      {
        printf("%c", c);
        input[index++] = c;
      }
